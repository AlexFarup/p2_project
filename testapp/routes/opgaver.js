...
router.get('/:opgavenummer', function(req, res, next) {
  var opgavenummer = parseInt(req.params.opgavenummer);
  ...
  con.connect(function(err) {
    ...
    con.query(`SELECT * 
    FROM \`Besvarelser\``, function (err, result, fields) {
        if (err) throw err;
        ...
        do {
          ...
          for(let opgavenummer = 0; opgavenummer < result.length; opgavenummer++){
            if(result[opgavenummer].Besv_Opgave_ID === opgavenummer && opgavenummer === taeller){
              opgave.push(opgavenummer);
            }
            opgavenummer = result.length;
          }
          taeller++;
          
        }while(taeller < result.length);


        for(let opgavenummer = 0; opgavenummer < opgave.length; opgavenummer++){
          for(let besvarelse = 0; besvarelse < result.length; besvarelse++){
            if (result[besvarelse].Besv_Besvaret === 1 && result[besvarelse].Besv_Opgave_ID === opgave[opgavenummer] ){
              besvaret.push(result[besvarelse].Besv_Besvaret);
            }
...
function kMeans (elever, opgaver, besvarelser){
